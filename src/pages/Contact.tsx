...
function FieldLabel({ children, htmlFor }: { children: React.ReactNode; htmlFor: string }) {
  return (
    <label htmlFor={htmlFor} className="mb-2 block font-mono text-[11px] uppercase tracking-[0.18em] text-(--color-fog)">
      {children}
    </label>
  );
}
    ...
    <PageShell>
      <section className="relative overflow-hidden pt-44 pb-28">
        <div
          ...
          style={{ background: "radial-gradient(55% 50% at 80% 0%, rgba(214,168,92,0.18), transparent 70%)" }}
        />
        <div className="mx-auto grid max-w-7xl gap-16 px-6 sm:px-10 lg:grid-cols-[1fr_1.15fr] lg:items-start">
          ...
          <div>
            ...
            <Reveal delay={0.1} className="relative mt-10 hidden h-72 sm:block">
              <AccentScene variant="field" color="#cfa563" className="h-full [&_canvas]:!h-full" />
            </Reveal>

            <Reveal delay={0.15} className="mt-10 space-y-4 font-mono text-sm text-(--color-fog)">
              <div className="flex items-center gap-3">
                <span className="h-px w-6 bg-(--color-gold)" />
                <a href="mailto:mouth2mouth@steevanz.com" className="transition-colors hover:text-(--color-bone)">mouth2mouth@steevanz.com</a>
              </div>
              <div className="flex items-center gap-3">
                <span className="h-px w-6 bg-(--color-gold)" />
                <a href="tel:+351928069072" className="transition-colors hover:text-(--color-bone)">+351 928 069 072</a>
              </div>
              <div className="flex items-center gap-3">
                <span className="h-px w-6 bg-(--color-gold)" />
              ...
              <div className="flex items-center gap-3">
                <span className="h-px w-6 bg-(--color-gold)" />
                <a href="https://www.instagram.com/steevanz_" target="_blank" rel="noreferrer" className="transition-colors hover:text-(--color-bone)">@Steevanz_</a>
              </div>
            </Reveal>
          </div>
